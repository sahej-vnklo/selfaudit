...
const THEMES = {
  ...
  elite: {
    ...
    bg: EMBER.page,
    surface: EMBER.band,
    surface2: EMBER.panel2,
    surface3: EMBER.panel,
    card: EMBER.panel,
    border: EMBER.line,
    border2: EMBER.lineStrong,
    ink: EMBER.ink,
    inkSoft: EMBER.soft,
    inkMuted: EMBER.muted,
    inkFaint: EMBER.faint,
    accent: EMBER.accent,
...
function PrimaryButton({ label, onClick, small = false, C }) {
  ...
  const darkTheme = C.theme === 'dark' || C.theme === 'elite'
  return (
    <button
      onClick={onClick}
...
function EngineRoom({ C }) {
  ...
  const softPalette = C.theme === 'elite' ? EMBER : C.theme === 'sharp' ? SHARP : LIGHT
...
function FourVerbs({ C }) {
  ...
  const cardBg = C.card
  const cardPanelBg = C.theme === 'sharp' || C.theme === 'elite' ? C.surface2 : C.surface
